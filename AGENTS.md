...
- `scripts/crosspost.ts`: run by the local `/crosspost` skill after a post is live. It publishes the full post to dev.to with the blog as canonical, taking the body from the post's live `.md` copy through `postBody` in `src/lib/llms.ts`, the reader beside the `postMarkdown` writer, and posts a Bluesky link card.
