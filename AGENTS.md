...
- `src/content/blog/*.md`: posts. Frontmatter: `title`, `description`, `date`, `tags`, optional `updated` and `devto`. A post with images lives in its own folder as `index.md`. The post id is the file or folder name and becomes `/blog/<id>/`, so an id cannot be numeric, `tag` or `year` (those are sub-routes; the dist test's feed count fails on a collision). `frontmatter.json` configures the Front Matter CMS extension for this folder; when the blog schema changes, update its fields too.
...
- `src/pages/`: routes. A new top-level page also touches `site.nav`, `src/data/banners.ts`, the Pages list in `src/lib/llms.ts`, `pageTypes` in `scripts/routes.ts` and, if it should be linked from there, `404.astro`. `blog/[...page]` paginates 10 per page; `blog/tag/[tag]` and `blog/year/[year]` filter; `og/[slug].png.ts` renders OG cards; `rss.xml.ts`, `atom.xml.ts`, `feed.json.ts`, `llms.txt.ts`, `llms-full.txt.ts` are text endpoints and must list every post (the dist test counts them).
- `src/layouts/Base.astro`: the only layout. Its inline script owns keyboard navigation and reads attributes from the page: `a[data-key]` binds a key to a link (nav, prev/next), `main a[data-item]` makes a row a `j`/`k` target, `[data-for]` help rows hide when their key is not bound on the page. New list rows and paginated links need these attributes to stay keyboard-reachable.
- `src/styles/global.css`: all design tokens live in `@theme`. Two places repeat palette hex values because they cannot read CSS: `theme-color` in `Head.astro` and `colors` in `src/lib/og.ts`; change them together. Sizes are rem only; the only px values are 1px borders.
- `scripts/crosspost.ts`: run by the local `/crosspost` skill after a post is live. It publishes the full post to dev.to with the blog as canonical, taking the body from the live `llms-full.txt` (`devtoBody` in `src/lib/crosspost.ts` parses its per-post `- URL:` header, so keep that format in sync), and posts a Bluesky link card.
